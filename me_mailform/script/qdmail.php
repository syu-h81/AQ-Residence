...
*/
	...
	function setMbParameter( $lang = null , $internal_enc = null , $detect = null ){
		...
		}elseif(!is_null($lang) && 'STACK'===strtoupper($lang) ){
...
##					list( $ad , $void ) = $this->keyUpper( $ad );
	...
	function send( $option = null ){
        ...
        if( !isset($other['DATE']) ){
            $this->other_header['Date'] =  date('D, d M Y H:i:s O');
        }
...
EOF;
	...
	function & smtpObject( $null = false ){
        ...
        // qdmail のエラー表示設定を qdsmtp に渡す
        $this->smtp_object->error_display = $this->error_display;
		return $this->smtp_object;
	}
